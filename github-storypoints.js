(function(d, w) {
  ...
  var pluralize = value => (value === 1 ? '' : 's');

  var resetStoryPointsForColumn = column => {
    const customElements = Array.from(
      column.getElementsByClassName('github-project-story-points')
    );
  ...
  var titleWithTotalPoints = (title, points, unestimated) => {
    ...
    if (unestimated > 0) {
      unestimated_element = `${unestimated} missing SP`;
    }
    ...
    return `${title} task${pluralize(
      title
    )} <span class="github-project-story-points" style="font-size:xx-small">(${points_element}${unestimated_element})</span>`;
  };

  var addStoryPointsForColumn = column => {
    const columnCards = Array.from(column.getElementsByClassName('issue-card'))
      .filter(
        // ignore filtered (invisible) cards
        card => !card.classList.contains('d-none')
      )
      .filter(
        // filter only to issues that are open
        card => card.getElementsByClassName('octicon-issue-opened').length > 0
      )
      .map(card => {
        const labels = Array.from(card.getElementsByClassName('labels'));

        const estimateLabels = Array.from(
          card.getElementsByClassName('issue-card-label')
        ...
        const match = estimateRegEx.exec(firstEstimateText) || [null, '0'];
    ...
    const columnCountElement = column.getElementsByClassName(
    ...
    if (columnStoryPoints || columnUnestimated) {
      columnCountElement.innerHTML = titleWithTotalPoints(
        columnCards.length,
        columnStoryPoints,
        columnUnestimated
      );
  ...
  var start = debounce(() => {
    ...
    if (projects.length > 0) {
      ...
      const columns = Array.from(
        project.getElementsByClassName('js-project-column')
      ); // Was 'col-project-custom', but that's gitenterprise; github.com is 'project-column', fortunately, both have 'js-project-column'
      for (let column of columns) {
        const addStoryPoints = (c =>
          debounce(() => {
